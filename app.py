...
from fetch_log import load_fetch_log
from fetcher import (
    MAX_QUERY_LENGTH,
    VALID_CHART_DAYS,
    fetch_coin_chart,
...
def get_admin_status() -> dict:
    """Combine cache snapshot, fetch log events, and record counts for the admin panel."""
    cache = load_cache()
    log = load_fetch_log()
    ...
    return {
        ...
        "total_records": coin_count + news_count,
        "last_updated": cache.get("last_updated") or "",
        "last_successful_fetch": cache.get("last_successful_fetch") or "",
...
@app.route("/admin")
def admin():
    return render_template("admin.html", status=get_admin_status())
...
def admin_refresh():
    result = refresh_cache(trigger="manual")
    if result.get("locked"):
        return jsonify({"error": "Refresh already in progress."}), 409
    payload = get_admin_status()
